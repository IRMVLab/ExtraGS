...
def readNerfbustersInfo_with_depth(path, images, depths, eval=True, downsample_factor=2, random_init=False, mode="default", oracle=False):
    ...
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
    # debug brief view of cam_infos
    for c in cam_infos[:5]:
        print(f"[DEBUG] : cam_info : {c.image_name} ; depth_path = {c.depth_path} ; depth_params = {c.depth_params}")
    print(f"[DEBUG] : total # cam_infos = {len(cam_infos)}")
    train_cam_infos = []
    test_cam_infos = []
    for idx, c in enumerate(cam_infos):
        if "frame_1_" in c.image_name:
