...
class SpaceSearch:
    ...
    def __init__(self, scene, gaussians, cam_generator, dataset, opt, pipe, render_func, render_fisher, diff_params, mask_params, cam_params):
        ...
        # Optional reweighting controls (default off to preserve existing behavior).
        self.ill_reweight_enable = getattr(cam_params, "search_ill_reweight", False)
        self.ill_lambda = float(getattr(cam_params, "search_ill_lambda", 0.0))
        self.ill_patch = int(getattr(cam_params, "search_ill_patch", 11))

        self.fold_penalty_enable = getattr(cam_params, "search_fold_penalty", False)
        self.fold_tau_deg = float(getattr(cam_params, "search_fold_tau_deg", 25.0))
        self.fold_eta = float(getattr(cam_params, "search_fold_eta", 0.7))
    ...
    def _get_cam_rgb(self, cam):
        """Return RGB tensor [3,H,W] for dark-region analysis."""
        if getattr(cam, "original_image", None) is not None:
            img = cam.original_image
            if torch.is_tensor(img):
                return img[:3].detach().float().cpu()

        with torch.no_grad():
            render_pkg = self.render_func(cam, self.gaussians, self.pipe, self.bg_color)
        return render_pkg["render"][:3].detach().float().cpu()

    def _dark_direction_world(self, cam):
        """Back-project darkest pixel/patch center to world-space unit ray."""
        rgb = self._get_cam_rgb(cam)
        gray = rgb.mean(dim=0, keepdim=True)  # [1,H,W]
        h, w = gray.shape[-2], gray.shape[-1]

        patch = max(1, int(self.ill_patch))
        if patch > 1:
            # Smooth darkness map by local patch average.
            pooled = F.avg_pool2d(gray[None], kernel_size=patch, stride=1, padding=patch // 2)[0, 0]
        else:
            pooled = gray[0]

        flat_idx = int(torch.argmin(pooled).item())
        py = flat_idx // w
        px = flat_idx % w
        ...
        # Camera convention in this codebase: forward is +Z.
        ray_cam = np.array([(px - cx) / fx, (py - cy) / fy, 1.0], dtype=np.float32)
        ray_cam = ray_cam / (np.linalg.norm(ray_cam) + _EPS)

        c2w = cam.c2w[:3, :3].detach().cpu().numpy()
        ray_world = c2w @ ray_cam
        ray_world = ray_world / (np.linalg.norm(ray_world) + _EPS)
        return ray_world

    def _motion_dir(self, cam_cur, cam_cand):
        c0 = cam_cur.c2w[:3, 3].detach().cpu().numpy()
        c1 = cam_cand.c2w[:3, 3].detach().cpu().numpy()
        d = c1 - c0
        n = np.linalg.norm(d)
        if n < _EPS:
            return None
    ...
    def _rotation_delta_deg(self, cam_cur, cam_cand):
        r0 = cam_cur.c2w[:3, :3].detach().cpu().numpy()
        r1 = cam_cand.c2w[:3, :3].detach().cpu().numpy()
        r_rel = r0.T @ r1
        tr = np.clip((np.trace(r_rel) - 1.0) * 0.5, -1.0, 1.0)
        return float(np.degrees(np.arccos(tr)))

    def _apply_ill_fold_reweight(self, base_gain, cam_cur, cam_cand, dark_dir_world=None):
        gain = float(base_gain)

        if self.ill_reweight_enable and dark_dir_world is not None and self.ill_lambda > 0.0:
            motion_dir = self._motion_dir(cam_cur, cam_cand)
            if motion_dir is not None:
                cos_sim = float(np.clip(np.dot(dark_dir_world, motion_dir), -1.0, 1.0))
                w_ill = 1.0 + self.ill_lambda * max(0.0, cos_sim)
                gain *= w_ill

        if self.fold_penalty_enable and self.fold_eta < 1.0:
            d_rot = self._rotation_delta_deg(cam_cur, cam_cand)
            if d_rot > self.fold_tau_deg:
                gain *= self.fold_eta
    ...
    def check_depths_health(depths, name="depths", verbose=True):
        """
        检查 depth list 是否为空、是否全零、是否无效、是否有 NaN/Inf。
        depths: list of torch.Tensor, each shape [1,H,W] or [H,W]
        """
        ...
        # 1️⃣ 空 list
        if depths is None:
            print("[ERROR] depths is None")
            return False

        if not isinstance(depths, (list, tuple)):
            print("[ERROR] depths is not a list/tuple")
            return False

        if len(depths) == 0:
            print("[ERROR] depths list is empty")
            return False

        print(f"[INFO] Number of depth frames: {len(depths)}")
        ...
        for i, d in enumerate(depths):
            ...
            if not torch.is_tensor(d):
                print(f"[ERROR] Frame {i} is not tensor")
                all_valid = False
                continue

            if d.numel() == 0:
                print(f"[ERROR] Frame {i} is empty tensor")
                all_valid = False
                continue

            if d.dim() not in [2, 3]:
                print(f"[WARNING] Frame {i} has unusual shape: {d.shape}")

            d_flat = d.reshape(-1)

            num_pixels = d_flat.numel()
            total_pixels += num_pixels

            num_zero = (d_flat == 0).sum().item()
            num_positive = (d_flat > 1e-6).sum().item()
            num_nan = torch.isnan(d_flat).sum().item()
            num_inf = torch.isinf(d_flat).sum().item()
            ...
            if verbose and i < 5:
                print(f"[Frame {i}] "
                    f"min={d_flat.min().item():.6f} "
                    f"max={d_flat.max().item():.6f} "
                    f"zero_ratio={num_zero/num_pixels:.4f} "
                    f"positive_ratio={num_positive/num_pixels:.4f}")

        # 全局统计
        print("\n----- Global Statistics -----")
        print(f"Total pixels: {total_pixels}")
        print(f"Zero ratio: {total_zero / total_pixels:.4f}")
        print(f"Positive ratio: {total_positive / total_pixels:.4f}")
        print(f"NaN count: {total_nan}")
        print(f"Inf count: {total_inf}")

        # 判断是否“空”
        if total_positive == 0:
            print("\n[ERROR] All depth values are zero or invalid.")
            all_valid = False

        if total_nan > 0:
            print("[WARNING] NaN detected in depth.")

        if total_inf > 0:
            print("[WARNING] Inf detected in depth.")
    ...
    def init_mesh(self):
        ...
        # self.check_depths_health(depths)
        mins = []
        for dep in depths:  # dep: [1,H,W] or [H,W]
            dep = dep.reshape(-1)
            valid = torch.isfinite(dep) & (dep > 1e-6)
            mins.append(dep[valid].quantile(0.01))
        depth_min_median = torch.stack(mins).median()
        # depth_min_median = torch.cat(depths).view(len(depths), -1).min(dim=1).values.median()
        print("[DEBUG] : depth_min_median = ", depth_min_median) # value=0? why
        depth_threshold = depth_min_median * 0.5
        # depth_threshold = depth_min_median
        ...
        for strategy in self.expand_strategy: # [Room for multiple strategies]
            ...
            for traj in trajectories:
                ...
                if "view-coverage-refine" == strategy or "view-coverage-grid-refine" == strategy:
                    ...
                    if traj.grid_view_dirs is None:
                        print("[WARN] : skip grid_view_dirs update because trajectory has no refine-state grid")
                        new_grid_sum = prev_grid_sum
                    else:
    ...
    def expand_trajectory_nbvs_dfs(self, traj, shared_K, params, I_train, min_unc=-3.0, max_unc=6.0):
        ...
        for idx, (candidate_c2w, name) in enumerate(zip(candidate_c2ws, names)):
            ...
            if name_rejected:
                rejection_stats["name_reverse"] += 1
                continue
            ...
            rejected, reject_reason = self.camera_rejection(v_cam, return_reason=True)
            if rejected:
        ...
        if len(candidate_cams) == 0:
            ...
            # print(f"[DEBUG] : rejection stats = {dict(rejection_stats)}")
    ...
    def expand_trajectory_view_coverage_dfs(self, traj, _, params, grid_view_dirs, view_dirs, grid_gs=None):
        ...
        if len(candidate_cams) == 0:
            ...
            self.debug_invalid_cam(cam)
            print(f"[DEBUG] : view_coverage : rejection stats = {dict(rejection_stats)}")
            return []
        
        sorted_idx = self.policy_view_coverage(params, candidate_cams, grid_view_dirs, view_dirs, grid_gs, current_cam=cam)
    ...
    def expand_trajectory_view_coverage_refine_dfs(self, traj, _, params, grid_view_dirs, view_dirs, grid_gs=None, use_grid=False):
        ...
        if len(candidate_cams) == 0:
            print("[DEBUG] : view_coverage : reject : no valid candidate cameras")
            # print(f"[DEBUG] : view_coverage_refine : rejection stats = {dict(rejection_stats)}")
            self.debug_invalid_cam(cam)
            return []
        
        sorted_idx, updated_grid_view_dirs_list = self.policy_view_coverage_refine(
            params,
            candidate_cams,
            grid_view_dirs,
            view_dirs,
    ...
    def policy_view_coverage(self, params, candidate_cams, grid_view_dirs, view_dirs, grid_gs=None, current_cam=None):
        """
        ...
        cam_cur = current_cam if current_cam is not None else (candidate_cams[0] if len(candidate_cams) > 0 else None)
        dark_dir_world = None
        if (self.ill_reweight_enable or self.fold_penalty_enable) and cam_cur is not None:
            dark_dir_world = self._dark_direction_world(cam_cur)
        ...
        for cam in candidate_cams:
            ...
            idx = np.argmax(angle_sim)
            base_gain = (grid_view_dirs[mask_visible, idx] == 0).sum()
            gain = self._apply_ill_fold_reweight(base_gain, cam_cur if cam_cur is not None else cam, cam, dark_dir_world)
            gains.append(gain)
    ...
    @torch.no_grad()
    def policy_view_coverage_refine(self, params, candidate_cams, grid_view_dirs, view_dirs, grid_gs=None, use_grid=False, current_cam=None):
        """
        grid_view_dirs : (N_grid, N_view)
        view_dirs : (N_view, 3)
        if use_grid:
        ...
        cam_cur = current_cam if current_cam is not None else (candidate_cams[0] if len(candidate_cams) > 0 else None)
        dark_dir_world = None
        if self.ill_reweight_enable or self.fold_penalty_enable:
            dark_dir_world = self._dark_direction_world(cam_cur) if cam_cur is not None else None
        ...
        for cam in candidate_cams:
            ...
            if self.gain_weight != "":
                if self.gain_weight == "visibility":
                    ...
                    base_gain = (new_grid_view_dirs.sum() - grid_view_dirs.sum()) * weight.mean().item()
                else:
                    base_gain = (new_grid_view_dirs.sum() - grid_view_dirs.sum())
            else:
                base_gain = (new_grid_view_dirs.sum() - grid_view_dirs.sum())
    ...
    def camera_rejection(self, cam, return_reason=False):
        
        if self.skip_rejection:
            return (False, "skip_rejection") if return_reason else False
        ...
        if self.skip_freespace_rejection:
            ...
            if not is_free:
                ...
                return (True, "free") if return_reason else True
        
        # print(f"[DEBUG] : camera_rejection : free space check passed")
        ...
        # print all the reject stats
        # print(f"[DEBUG] : camera_rejection : reject_bg = {reject_bg}")
        # print(f"[DEBUG] : camera_rejection : bg_mask.sum() = {bg_mask.sum()}")

        if reject_bg:
            save_rejected_cam(cam, suffix="bg")
            return (True, "bg") if return_reason else True
        else:
            reject_depth, depth_mask = self.reject_cam_by_depth(render_reject_pkg, bg_mask)
            if reject_depth:
                save_rejected_cam(cam, suffix="depth")
                return (True, "depth") if return_reason else True
            else:
                return (False, "pass") if return_reason else False
