...
# Ensure we can import COLMAP read/write utilities from the repository utils.
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_EXPLORGS_ROOT = os.path.abspath(os.path.join(_THIS_DIR, "../../.."))
_UTILS_DIR = os.path.join(_EXPLORGS_ROOT, "utils")
...
def get_scales(key, cameras, images, points3d_ordered, args):
    ...
    if len(pts_idx) > 0:
        ...
        return {"image_name": os.path.splitext(image_meta.name)[0], "scale": 0, "offset": 0}

    R = qvec2rotmat(image_meta.qvec)
    pts = np.dot(pts, R.T) + image_meta.tvec

    invcolmapdepth = 1. / pts[..., 2]
    image_stem = os.path.splitext(image_meta.name)[0]
    invmonodepthmap = cv2.imread(f"{args.depths_dir}/{image_stem}.png", cv2.IMREAD_UNCHANGED)
    ...
    if valid.sum() > 10 and (invcolmapdepth.max() - invcolmapdepth.min()) > 1e-3:
        ...
        mapx = maps[..., 0].reshape(1, -1)
        mapy = maps[..., 1].reshape(1, -1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base_dir', default="", help="If set, override computed dataset scene path")
    parser.add_argument('--depths_dir', default="", help="If set, override depth folder (e.g., inv_depths)")
